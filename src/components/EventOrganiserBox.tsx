"use client";
import React from "react";
import { EntryPlaceEvent } from "../api/FetchEvent";
import { Button } from "./Button";
import Link from "next/link";

interface EventLocationBoxProps {
  event: EntryPlaceEvent;
}

const EventOrganiserBox = ({ event }: EventLocationBoxProps) => {
  const { location_type, location_name, location_address, latitude, longitude, title, online_description } = event;

  const hasCoordinates = latitude && longitude;

  const mapImageUrl = hasCoordinates
    ? `https://maps.googleapis.com/maps/api/staticmap?scale=2&center=${latitude},${longitude}&zoom=15&maptype=roadmap&size=640x320&format=png&scale=2&key=${process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY}&markers=size%3Asmall%7Ccolor%3A0xe0cce9%7Ccolor%3A0xF65858%7C${latitude},${longitude}`
    : null;

  const websiteLink = event.organiser_website_link;
  const facebookLink = event.organiser_facebook_link;
  const instagramLink = event.organiser_instagram_link;

  return (
    <div>
      <div className="flex flex-row gap-4 align-items-center border-b border-gray-100 pb-4">
        {event.organiser_image_url && (
        <div className="h-16 w-16 flex-shrink-0">
          <Link href={`/organiser/${event.event_organiser_slug}`}>
            <img 
            className="img-fluid rounded-lg h-16 w-16" 
            src={event.organiser_image_url}
            alt="avatar" 
            />
          </Link>
          </div>
        )}

        <div className="flex flex-col justify-center">
          <h4 className="text-gray-700 font-medium hover:underline">
            <Link href={`/organiser/${event.event_organiser_slug}`}>{event.organiser_name}</Link>
            </h4>
          {event.entries_contact_details && 
            <div className="prose prose-sm prose-indigo pb-4" dangerouslySetInnerHTML={{__html:event.entries_contact_details}}>
            </div>
          }
        </div>

      </div>

      {websiteLink || facebookLink || instagramLink ? (
      <div className="flex flex-col sm:flex-row gap-2 pb-4 border-b border-gray-100">
        {websiteLink && (
          <Button variant='outline' href={websiteLink} target='_blank' className='mt-4 me-2 flex gap-2'>
            <svg className='size-5' data-slot="icon" fill="none" strokeWidth="1.5" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
              <path strokeLinecap="round" strokeLinejoin="round" d="M12 21a9.004 9.004 0 0 0 8.716-6.747M12 21a9.004 9.004 0 0 1-8.716-6.747M12 21c2.485 0 4.5-4.03 4.5-9S14.485 3 12 3m0 18c-2.485 0-4.5-4.03-4.5-9S9.515 3 12 3m0 0a8.997 8.997 0 0 1 7.843 4.582M12 3a8.997 8.997 0 0 0-7.843 4.582m15.686 0A11.953 11.953 0 0 1 12 10.5c-2.998 0-5.74-1.1-7.843-2.918m15.686 0A8.959 8.959 0 0 1 21 12c0 .778-.099 1.533-.284 2.253m0 0A17.919 17.919 0 0 1 12 16.5c-3.162 0-6.133-.815-8.716-2.247m0 0A9.015 9.015 0 0 1 3 12c0-1.605.42-3.113 1.157-4.418"></path>
            </svg>
            Website
          </Button>
        )}
        {facebookLink && (
          <Button variant='outline' href={facebookLink} target='_blank' className='mt-4 me-2 flex gap-2'>
            <svg className="size-5" fill="currentColor" viewBox="0 0 24 24">
              <path fillRule="evenodd" d="M22 12c0-5.523-4.477-10-10-10S2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.878v-6.987h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.562V12h2.773l-.443 2.89h-2.33v6.988C18.343 21.128 22 16.991 22 12z" clipRule="evenodd"></path>
            </svg>
            Facebook
          </Button>
        )}
        {instagramLink && (
        <Button variant='outline' href={instagramLink} target='_blank' className='mt-4 me-2 flex gap-2'>
          <svg className="size-5" fill="currentColor" viewBox="0 0 24 24">
            <path fillRule="evenodd" d="M12.315 2c2.43 0 2.784.013 3.808.06 1.064.049 1.791.218 2.427.465a4.902 4.902 0 011.772 1.153 4.902 4.902 0 011.153 1.772c.247.636.416 1.363.465 2.427.048 1.067.06 1.407.06 4.123v.08c0 2.643-.012 2.987-.06 4.043-.049 1.064-.218 1.791-.465 2.427a4.902 4.902 0 01-1.153 1.772 4.902 4.902 0 01-1.772 1.153c-.636.247-1.363.416-2.427.465-1.067.048-1.407.06-4.123.06h-.08c-2.643 0-2.987-.012-4.043-.06-1.064-.049-1.791-.218-2.427-.465a4.902 4.902 0 01-1.772-1.153 4.902 4.902 0 01-1.153-1.772c-.247-.636-.416-1.363-.465-2.427-.047-1.024-.06-1.379-.06-3.808v-.63c0-2.43.013-2.784.06-3.808.049-1.064.218-1.791.465-2.427a4.902 4.902 0 011.153-1.772A4.902 4.902 0 015.45 2.525c.636-.247 1.363-.416 2.427-.465C8.901 2.013 9.256 2 11.685 2h.63zm-.081 1.802h-.468c-2.456 0-2.784.011-3.807.058-.975.045-1.504.207-1.857.344-.467.182-.8.398-1.15.748-.35.35-.566.683-.748 1.15-.137.353-.3.882-.344 1.857-.047 1.023-.058 1.351-.058 3.807v.468c0 2.456.011 2.784.058 3.807.045.975.207 1.504.344 1.857.182.466.399.8.748 1.15.35.35.683.566 1.15.748.353.137.882.3 1.857.344 1.054.048 1.37.058 4.041.058h.08c2.597 0 2.917-.01 3.96-.058.976-.045 1.505-.207 1.858-.344.466-.182.8-.398 1.15-.748.35-.35.566-.683.748-1.15.137-.353.3-.882.344-1.857.048-1.055.058-1.37.058-4.041v-.08c0-2.597-.01-2.917-.058-3.96-.045-.976-.207-1.505-.344-1.858a3.097 3.097 0 00-.748-1.15 3.098 3.098 0 00-1.15-.748c-.353-.137-.882-.3-1.857-.344-1.023-.047-1.351-.058-3.807-.058zM12 6.865a5.135 5.135 0 110 10.27 5.135 5.135 0 010-10.27zm0 1.802a3.333 3.333 0 100 6.666 3.333 3.333 0 000-6.666zm5.338-3.205a1.2 1.2 0 110 2.4 1.2 1.2 0 010-2.4z" clipRule="evenodd"></path>
          </svg>
          Instagram
        </Button>
        )}
      </div>
      ) : (
        <></>
      )}
      <div>
        {event.organiser_description && 
          <div className="mt-4 prose prose-sm prose-indigo mb-4" dangerouslySetInnerHTML={{__html:event.organiser_description}}>
          </div>
        }
      </div>
    </div>
  );
};

export default EventOrganiserBox;